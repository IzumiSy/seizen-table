...
{/* Accessible description for AI/screen readers: DataTable layout has left and right sidepanels, a header slot above the table body, an inlineRow slot that expands below each row, a cell slot for custom cell rendering, and a footer slot below the table. */}
