...
![DataTable plugin slots layout](/seizen-table/slots-diagram.svg)
...
```tsx
import { z } from "zod";
import { definePlugin } from "@izumisy/seizen-table-plugins";
...
export const MyPlugin = definePlugin({
  ...
  slots: { /* ... */ },      // UI slots (sidePanel, header, footer, cell, inlineRow)
...
function createSidePanelRenderer(context: PluginContext<SidePanelConfig>) {
  const { args } = context;
...
<Aside type="tip">
  Custom events are useful for decoupling plugins. For example, a context menu plugin can emit a filter request, and the filter plugin can subscribe to it without direct dependency.
</Aside>
...
function createSidePanelRenderer(context: PluginContext<BulkActionsConfig>) {
  const { args } = context;
